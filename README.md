...
In addition to an occupancy sensor for each side of the bed, there are two additional sensors: one checks if either side of the bed is occupied, and the other checks if both sides are occupied. These are convenient for automations, but they (or any other sensor/control) can be hidden if you don't intend to use them (coming soon). 
